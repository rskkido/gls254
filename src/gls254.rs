...
impl Point {
    ...
    #[inline(always)]
    fn set_add_withb_toJaco(&mut self, rhs: &Self) {
        let (X1, S1, Z1, T1) = (&self.X, &self.S, &self.Z, &self.T);
        let (X2, S2, Z2, T2) = (&rhs.X, &rhs.S, &rhs.Z, &rhs.T);
    ...
    #[inline(always)]
    fn set_add_affine_withb_toJaco(&mut self, rhs: &PointAffine) {
        // This is similar to set_add() except that:
        //   Z2 = 1
        //   T2 = X2
        // cost: 7M+2S
    ...
    #[allow(dead_code)]
    fn add_sub_affine_toJaco(self, rhs: &PointAffine) -> (Self, Self) {
        // Starting from set_add_affine(), we also compute self - rhs, with:
        //   (-rhs).scaled_s = rhs.scaled_s + rhs.scaled_x.
    ...
    /// Note: the Z coordinate of the output point is in GF(2^127).
    #[allow(dead_code)]
    fn add_affine_selfzeta_toJaco(P: &PointAffine, neg: u32) -> Self {
        // This is similar to set_add_affine_affine() except that:
        //   X2 = phi(X1)
        //   S2 = phi(S1) + (u + 1)*phi(X1)
        // If neg != 0, we also negate the point afterwards.
    ...
    /// Note: the Z coordinate of the output point is in GF(2^127).
    #[allow(dead_code)]
    fn set_add_selfzeta_toJaco(&mut self, neg: u32) {
        let (X1, S1, Z1, T1) = (&self.X, &self.S, &self.Z, &self.T);
        let rhs = self.zeta(neg);
        let (S2, T2) = (&rhs.S, &rhs.T);
    ...
    /// Moreover, the two returned point have the same Z coordinate.
    #[allow(dead_code)]
    fn add_sub_affine_affine_toJaco(P1: &PointAffine, P2: &PointAffine)
        -> (Self, Self)
    {
        // Starting from add_affine_affine(), we also compute P1 - P2 with:
        //   (-P2).scaled_s = P2.scaled_s + P2.scaled_x.
    ...
    /// Doubles this point (in place).
    pub fn double_toJaco(&self) -> Self {
        // Note: this implementation uses the 3M+4S formulas. The
        // formulas used in set_xdouble() would yield a cost of 2M+6S,
        // which might be faster on architectures without a carryless
        // multiplication opcode, and where GF(2^254) multiplications
    ...
    #[inline(always)]
    fn xdouble_affine_withb(n: u32, rhs: &PointAffine) -> Self {
        let (X1, S1) = (&rhs.scaled_x, &rhs.scaled_s);
        
        let mut X = X1.mul_sb();
        let mut T = X;
        X = X.square();
        let mut Y = S1.mul_sb() + X + T.mul_u();
    ...
    #[cfg(feature = "gls254bench")]
    #[inline(always)]
    fn set_add_xdouble_withb(&mut self, n: u32, P: &Point) {
        let (X1, S1, Z1, T1) = (&self.X, &self.S, &self.Z, &self.T);
        let (X2, S2, Z2, T2) = (&P.X, &P.S, &P.Z, &P.T);
        ...
        let G = Z1Z2.square();
        let X3 = D + S1S2;
        let S3 = G * (S1S2 + E + G) + F * (D + E + F); // S3 + Z^2
        let Z3 = F + G;
    ...
    #[cfg(feature = "gls254bench")]
    #[inline(always)]
    fn set_add_xdouble_affine_withb(&mut self, n: u32, rhs: &PointAffine) {
        let (X1, S1, Z1, T1) = (&self.X, &self.S, &self.Z, &self.T);
        let (X2, S2) = (&rhs.scaled_x, &rhs.scaled_s);
    ...
    #[cfg(feature = "gls254bench")]
    #[inline(always)]
    fn set_add_xdouble_affine_withb_c0(&mut self, n: u32, rhs: &PointAffine) {
        let (X1, S1, Z1, T1) = (&self.X, &self.S, &self.Z, &self.T);
        let (X2, S2) = (&rhs.scaled_x, &rhs.scaled_s);
        ...
        self.Z = X.mul_sb();
        self.T = Z * self.Z;
        self.S = Y.mul_sb() + self.T.mul_u1(); //
    }

    #[cfg(feature = "gls254bench")]
    #[inline(always)]
    fn set_add_xdouble_affine_withb_c1(&mut self, n: u32, rhs: &PointAffine) {
        let (X1, S1, Z1, T1) = (&self.X, &self.S, &self.Z, &self.T);
        let (X2, S2) = (&rhs.scaled_x, &rhs.scaled_s);
        ...
        let G = Z1.square();
        let X3 = D + S1S2;
        let S3 = G * (D + E + G) + F * (S1S2 + E + F); // S3 + Z^2 + T
        let Z3 = F + G;
        ...
        self.Z = X.mul_sb();
        self.T = Z * self.Z;
        self.S = Y.mul_sb() + self.T.mul_u(); //
    }

    #[cfg(feature = "gls254bench")]
    #[inline(always)]
    fn set_add_xdouble_affine_affine_withb(&mut self, n: u32, P1: &PointAffine, P2: &PointAffine) {
        let (X1, S1) = (&P1.scaled_x, &P1.scaled_s);
        let (X2, S2) = (&P2.scaled_x, &P2.scaled_s);
        ...
        let X3 = D + S1S2;
        let Fb = F.mul_b();
        let S3 = (S1S2 + E + F * (D + E + Fb) +  &Self::B).mul_b(); // S3 + Z^2
        let Z3 = Fb + &Self::B;
        //let T3 = X3 * Z3;
    ...
    #[inline(always)]
    fn add_xdouble_affine_affine_withb(n: u32, P1: &PointAffine, P2: &PointAffine) -> Self {
        let mut r = Self::NEUTRAL;
        r.set_add_xdouble_affine_affine_withb(n, &P1, &P2);
        r
    }
    ...
    */
    ...
    #[allow(dead_code)]
    fn to_affine_fromJaco(self) -> PointAffine {
        let iZ = self.Z.invert();
        let iZZ = iZ.square();
        let qx = self.X * iZ;
        let qs = self.S * iZZ;
    ...
    /// This function uses a one-dimensional lookup table, and processes
    /// scalar bits in groups of 3.
    /// This function is an improved version of for_benchmarks_only_1dt_3.
    #[cfg(feature = "gls254bench")]
    pub fn for_benchmarks_only_AD_1dt_3(pp: &[u8], sk: &[u8]) -> Option<[u8; 64]> {
        // Decode the input point in scaled affine coordinates; fail if
        // the decoding fails (invalid representation of field elements)
        // or if the resulting point is not on the curve, or is not in
        // the proper group (the points P+N, for P \in E[r]).
        ...
        let mut win_ex = [Point::NEUTRAL; 4];
        Pa.set_condneg(s0);
        (win_ex[1], win_ex[2]) = Self::double_and_triple_affine_toJaco(&Pa);
        win_ex[3] = win_ex[1].double_toJaco();

        // Batch inversion for normalization. Since Z != 0 for all points
        // (including the neutral), this always works. The first point
        // is Pa and is already affine.
        let mut win = [GFb254::ZERO; 8];
        let z = win_ex[1].Z * win_ex[2].Z;
        let mut iz = (z * win_ex[3].Z).invert();
        let iZ = iz * z;
        let iZZ = iZ.square();
        win[6] = win_ex[3].X * iZ;
        win[7] = win_ex[3].S * iZZ;
        iz *= win_ex[3].Z;
        let iZ = iz * win_ex[1].Z;
        let iZZ = iZ.square();
        win[4] = win_ex[2].X * iZ;
        win[5] = win_ex[2].S * iZZ;
        let iZ = iz * win_ex[2].Z;
        let iZZ = iZ.square();
        win[2] = win_ex[1].X * iZ;
        win[3] = win_ex[1].S * iZZ;
        win[0] = Pa.scaled_x;
        win[1] = Pa.scaled_s;
        ...
        let Q0 = Self::lookup4_affine(&win, sd0[42]);
        let Q1 = Self::lookup4_affine_zeta(&win, sd1[42], zn);
        let mut P = Self::add_xdouble_affine_affine_withb(3, &Q0, &Q1);
        for i in (1..42).rev() {
            let Q0 = Self::lookup4_affine(&win, sd0[i]);
            let Q1 = Self::lookup4_affine_zeta(&win, sd1[i], zn);
            P.set_add_xdouble_withb(3, &Self::add_affine_affine(&Q0, &Q1));
        }
        let Q0 = Self::lookup4_affine(&win, sd0[0]);
        let Q1 = Self::lookup4_affine_zeta(&win, sd1[0], zn);
        P.set_add_withb_toJaco(&Self::add_affine_affine(&Q0, &Q1));

        let Qa = P.to_affine_fromJaco();
        let mut qq = [0u8; 64];
        qq[..32].copy_from_slice(&Qa.scaled_x.encode());
        qq[32..].copy_from_slice(&Qa.scaled_s.encode());
        Some(qq)
    ...
    /// This function uses a one-dimensional lookup table, and processes
    /// scalar bits in groups of 4.
    /// This function is an improved version of for_benchmarks_only_1dt_4.
    #[cfg(feature = "gls254bench")]
    pub fn for_benchmarks_only_AD_1dt_4(pp: &[u8], sk: &[u8]) -> Option<[u8; 64]> {
        // Decode the input point in scaled affine coordinates; fail if
        // the decoding fails (invalid representation of field elements)
        // or if the resulting point is not on the curve, or is not in
        // the proper group (the points P+N, for P \in E[r]).
        ...
        win_ex[3] = win_ex[1].double();
        win_ex[5] = win_ex[2].double();
        (win_ex[6], win_ex[4]) = win_ex[5].add_sub_affine_toJaco(&Pa);
        win_ex[7] = win_ex[3].double_toJaco();
        ...
        for i in (2..8).rev() {
            match i {
                ...
                4 => {
                    ...
                    let iZZ = iZ.square();
                    z *= win_ex[i].Z;
                    win[(2 * i) + 0] = win_ex[i].X * iZ;
                    win[(2 * i) + 1] = win_ex[i].S * iZZ;
                    win[(2 * i) + 4] = win_ex[i + 2].X * iZ;
                    win[(2 * i) + 5] = win_ex[i + 2].S * iZZ;
                },
                _ => {
                    let iZ = z * win[2 * i];
                    let iZZ = iZ.square();
        ...
        let Q0 = Self::lookup8_affine(&win, sd0[31]);
        let Q1 = Self::lookup8_affine_zeta(&win, sd1[31], zn);
        let mut P = Self::add_xdouble_affine_affine_withb(4, &Q0, &Q1);
        for i in (1..31).rev() {
            let Q0 = Self::lookup8_affine(&win, sd0[i]);
            let Q1 = Self::lookup8_affine_zeta(&win, sd1[i], zn);
            P.set_add_xdouble_withb(4, &Self::add_affine_affine(&Q0, &Q1));
        }
        let Q0 = Self::lookup8_affine(&win, sd0[0]);
        let Q1 = Self::lookup8_affine_zeta(&win, sd1[0], zn);
        P.set_add_withb_toJaco(&Self::add_affine_affine(&Q0, &Q1));
    ...
    /// This function uses a one-dimensional lookup table, and processes
    /// scalar bits in groups of 5.
    /// This function is an improved version of for_benchmarks_only_1dt_5.
    #[cfg(feature = "gls254bench")]
    pub fn for_benchmarks_only_AD_1dt_5(pp: &[u8], sk: &[u8]) -> Option<[u8; 64]> {
        // Decode the input point in scaled affine coordinates; fail if
        // the decoding fails (invalid representation of field elements)
        // or if the resulting point is not on the curve, or is not in
        // the proper group (the points P+N, for P \in E[r]).
        ...
        win_ex[7] = win_ex[3].double();
        win_ex[9] = win_ex[4].double();
        (win_ex[10], win_ex[8]) = win_ex[9].add_sub_affine_toJaco(&Pa);
        win_ex[11] = win_ex[5].double_toJaco();
        win_ex[13] = win_ex[6].double();
        (win_ex[14], win_ex[12]) = win_ex[13].add_sub_affine_toJaco(&Pa);
        win_ex[15] = win_ex[7].double_toJaco();
        ...
        let Q0 = Self::lookup16_affine(&win, sd0[25]);
        let Q1 = Self::lookup16_affine_zeta(&win, sd1[25], zn);
        let mut P = Self::add_xdouble_affine_affine_withb(5, &Q0, &Q1);
        for i in (1..25).rev() {
            let Q0 = Self::lookup16_affine(&win, sd0[i]);
            let Q1 = Self::lookup16_affine_zeta(&win, sd1[i], zn);
            P.set_add_xdouble_withb(5, &Self::add_affine_affine(&Q0, &Q1));
        }
        let Q0 = Self::lookup16_affine(&win, sd0[0]);
        let Q1 = Self::lookup16_affine_zeta(&win, sd1[0], zn);
        P.set_add_withb_toJaco(&Self::add_affine_affine(&Q0, &Q1));
    ...
    /// This function uses a two-dimensional lookup table, and processes
    /// scalar bits in groups of 2.
    /// This function is an improved version of for_benchmarks_only_2dt_2.
    #[cfg(feature = "gls254bench")]
    pub fn for_benchmarks_only_AD_2dt_2(pp: &[u8], sk: &[u8]) -> Option<[u8; 64]> {
        // Decode the input point in scaled affine coordinates; fail if
        // the decoding fails (invalid representation of field elements)
        // or if the resulting point is not on the curve, or is not in
        // the proper group (the points P+N, for P \in E[r]).
        ...
        let zP = P.zeta(nz);
        let P3 = Self::triple_affine(&P);
        let Q0 = Self::add_affine_selfzeta_toJaco(&P, nz);
        let (Q1, Q2) = P3.add_sub_affine_toJaco(&zP);
        let mut Q3 = P3;
        Q3.set_add_selfzeta_toJaco(nz);
        ...
        // Process the two digit sequences in high-to-low order.
        let mut Q = Self::xdouble_affine_withb(2, &lookup_2dt(&win, nz, sd0[63], sd1[63]));
        for i in (1..63).rev() {
            Q.set_add_xdouble_affine_withb(2,
                &lookup_2dt(&win, nz, sd0[i], sd1[i]));
        }
    ...
    /// This function uses a two-dimensional lookup table, and processes
    /// scalar bits in groups of 3.
    /// This function is an improved version of for_benchmarks_only_2dt_3.
    #[cfg(feature = "gls254bench")]
    pub fn for_benchmarks_only_AD_2dt_3(pp: &[u8], sk: &[u8]) -> Option<[u8; 64]> {
        // Decode the input point in scaled affine coordinates; fail if
        // the decoding fails (invalid representation of field elements)
        // or if the resulting point is not on the curve, or is not in
        // the proper group (the points P+N, for P \in E[r]).
        ...
        let R1 = P;
        let P3 = Self::triple_affine(&P);
        let (P7, P5) = P3.double().add_sub_affine_toJaco(&P);
        // Note: P5 and P7 come from add_sub_affine(); they have the same Z.
        let z = (P3.Z * P5.Z).invert();
        let iZ = z * P3.Z;
        let iZZ = iZ.square();
        let R7 = PointAffine {
            scaled_x: P7.X * iZ,
        ...
        // Diagonal elements: i*P + j*zeta(P)
        win_ex[0] = Self::add_affine_selfzeta_toJaco(&R1, nz);
        win_ex[5] = Self::add_affine_selfzeta_toJaco(&R3, nz);
        win_ex[10] = Self::add_affine_selfzeta_toJaco(&R5, nz);
        win_ex[15] = Self::add_affine_selfzeta_toJaco(&R7, nz);
        ...
        for i in (1..16).rev() {
            match i {
                ...
                _ => {
                    ...
                    match i {
                        4 => {
                            let mut P = PointAffine { scaled_x: win_ex[1].X * iZ, scaled_s: win_ex[1].S * iZZ };
                            P.set_zeta(nz);
                            win[(2 * 1) + 0] = P.scaled_x;
                            win[(2 * 1) + 1] = P.scaled_s;
                        },
                        8 => {
                            let mut P = PointAffine { scaled_x: win_ex[2].X * iZ, scaled_s: win_ex[2].S * iZZ };
                            P.set_zeta(nz);
                            win[(2 * 2) + 0] = P.scaled_x;
                            win[(2 * 2) + 1] = P.scaled_s;
                        },
                        12 => {
                            let mut P = PointAffine { scaled_x: win_ex[3].X * iZ, scaled_s: win_ex[3].S * iZZ };
                            P.set_zeta(nz);
                            win[(2 * 3) + 0] = P.scaled_x;
                            win[(2 * 3) + 1] = P.scaled_s;
                        },
                        9 => {
                            let mut P = PointAffine { scaled_x: win_ex[6].X * iZ, scaled_s: win_ex[6].S * iZZ };
                            P.set_zeta(nz);
                            win[(2 * 6) + 0] = P.scaled_x;
                            win[(2 * 6) + 1] = P.scaled_s;
                        },
                        13 => {
                            let mut P = PointAffine { scaled_x: win_ex[7].X * iZ, scaled_s: win_ex[7].S * iZZ };
                            P.set_zeta(nz);
                            win[(2 * 7) + 0] = P.scaled_x;
                            win[(2 * 7) + 1] = P.scaled_s;
                        },
                        14 => {
                            let mut P = PointAffine { scaled_x: win_ex[11].X * iZ, scaled_s: win_ex[11].S * iZZ };
                            P.set_zeta(nz);
                            win[(2 * 11) + 0] = P.scaled_x;
                            win[(2 * 11) + 1] = P.scaled_s;
                        },
                        _ => (),
                    }
                },
        ...
        // Process the two digit sequences in high-to-low order.
        let mut Q = Self::xdouble_affine_withb(3, &lookup_2dt(&win, nz, sd0[42], sd1[42]));
        for i in (1..42).rev() {
            Q.set_add_xdouble_affine_withb(3,
                &lookup_2dt(&win, nz, sd0[i], sd1[i]));
        }
        Q.set_add_affine_withb_toJaco(&lookup_2dt(&win, nz, sd0[0], sd1[0]));
